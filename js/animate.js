...
function ajaxRequestPerson(first_name, second_name) {
  $.ajax({
    ...
    success: function(response) {
      if (response == null) {
        if (english_lang_selected) {
          alert('Unable to get data');
        } else {
          alert('Dohvat podataka neuspješan');
        }
      } else {
        //set css margin url timeline here inside the if
