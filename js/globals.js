...
function createPost(i, cont, url, date, heading_name) {
  ...
  var moment = date[i]
    .toString()
    .substr(5, date[i].length)
    .trim();
  $(makediv)
    .find('.date-post')
    .text(moment.substring(moment.indexOf('-') + 1, moment.length) + '.' + moment.substring(0, moment.indexOf('-')) + '.');
  $(makediv)
    .find('p')
    .text(cont[i]);
  if (heading_name[i] == null) {
    $(makediv)
      .find('.heading-response')
      .text('... ');
  } else {
    $(makediv)
      .find('.heading-response')
...
function trendingFindOnline(htmltext, input_array_all) {
  ...
  for (var i = 0; i < input_array_all.length; i++) {
    if (
      htmltext.indexOf(input_array_all[i].prezime) > -1 &&
      input_array_all[i].prezime != 'Mesić' &&
      input_array_all[i].prezime != 'Grabar-Kitarović' &&
      input_array_all[i].prezime != 'Josipović'
    ) {
      //if contains
...
function removeDiacritics(input_word) {
  for (var i = 0; i < input_word.length; i++) {
    if (input_word[i] == 'ć' || input_word[i] == 'č') {
      input_word = input_word.replace(input_word[i], 'c');
    }
    if (input_word[i] == 'ž') {
      input_word = input_word.replace(input_word[i], 'z');
    }
    if (input_word[i] == 'š') {
      input_word = input_word.replace(input_word[i], 's');
    }
    if (input_word[i] == 'đ') {
      input_word = input_word.replace(input_word[i], 'd');
    }
    if (input_word[i] == ' ') {
      input_word = input_word.replace(input_word[i], '_');
    }
  }
  return input_word;
}
...
function populateMiddleIcons(arraynamelast, arrayimg, filter_word) {
  $('.ikonice').each(function(i, obj) {
    if (filter_word == 'trend') {
      $(this)
        .find('.txtimg')
        .text(arraynamelast[i].substring(arraynamelast[i].indexOf(' ') + 1, arraynamelast[i].length) + ' ' + arraynamelast[i][0] + '.');
    } else {
      $(this)
        .find('.txtimg')
        .text(
          arraynamelast[i].imprezim.substring(arraynamelast[i].imprezim.indexOf(' ') + 1, arraynamelast[i].imprezim.length) + ' ' + arraynamelast[i].imprezim[0] + '.'
        );
    }
    $(this)
      .find('img')
...
function populateMainMiddleObjectArrayTrendingOnly(reference_array) {
  var output_array = [];
  $('.isotope > .ikonice').each(function(i, obj) {
    if (i > trending_array_both.length - 1) {
      return false;
    }
    populatingMainArray(this, output_array, reference_array);
  });
...
function populatingMainArray(this_refference, array_general, reference_array) {
  for (var i = 0; i < reference_array.length; i++) {
    if (
      $(this_refference)
        .find('img')
        .attr('alt') == reference_array[i].imprezim
    ) {
      inside_main_middle = {};
