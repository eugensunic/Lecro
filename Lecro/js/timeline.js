$(document).ready(function () {
    ...
    $('.timeline').on('click','.year',function () {
        var indexofli = $(this).parent().children().index(this);
      
        if ($(".timeline li").eq(indexofli + 1).is(":hidden")) {
            while (true) {
                indexofli++;
                $(".timeline li").eq(indexofli).show();
                if (indexofli > $(".timeline li").last().index()) { break; }
                var temp_li = $('.timeline li').eq(indexofli).attr('class').toString().trim();
                if (temp_li == "year") { break; } 
    ...
    $('.dropping-years').on('change', function () {
        ...
        $(".timeline li").each(function (index, value) {
            if ($('.dropping-years option:selected').text().trim() == $(".timeline li").eq(index).text().trim()) {
                var pos_li = $(".timeline li").eq(index).position();
