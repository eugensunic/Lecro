...
function trendingIconsHover(ID) {  //ovo sa css
    $('#' + ID).show();
...
function getCategoryPeople(array,source,array_names0) {
    $(".ikonice").each(function (i, obj) {
        ...
        if (i < array_names0.length) {
            $(this).find(".txtimg").text(array_names0[i].newnames.substring(array_names0[i].newnames.indexOf(' ') + 1, array_names0[i].newnames.length) + " " + (array_names0[i].newnames)[0] + ".");
            $(this).find("img").attr({
...
function getCategoryPeopleAtStart() {  //just fetchin the img src values... you should also fetch the rest...
    var arrayintern = [];
    $(".ikonice").each(function (i, obj) {
        arrayintern[i] = $(this).find("img").attr("src");
...
function closure(storedVariable,array,source2,array_names) {  // da izbjegnem visak ifova
