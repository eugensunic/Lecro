...
$(document).ready(function () {
    $('.typeahead').on('typeahead:selected', function (obj, obj_value) {
        ...
        for (var i = 0; i < drzavnici.length; i++) {
            ...
            if (drzavnici[i].ime.trim() == first_name.trim() && drzavnici[i].prezime.trim() == last_name.trim()) {  //ivana maletic ne radi ocito zbog spellinga
              
                drzavnici_main_middle.splice(0, 0,          //ulazak typehead objekta unutra.
                {
                 "ime": first_name,
                 "prezime": last_name,
        ...
        $(makediv).find("img").attr({
            alt: alt_atb,
            src: "images/" + selected_item + ".jpg"
        });

        $(makediv).find(".txtimg").text(text_between);
        if (bar_color == "silver") {
            if (ustedevina) {
                    $(makediv).find(".textvalue").text(drzavnici_main_middle[0].ustedevina);
            }
            else if (nekretnine) {
                    $(makediv).find(".textvalue").text(drzavnici_main_middle[0].nekretnine);
            }
            else if (placa) {
                    $(makediv).find(".textvalue").text(drzavnici_main_middle[0].placa);
            }
            else if (broj_nekretnina) {
                    $(makediv).find(".textvalue").text(drzavnici_main_middle[0].broj_nekretnina);
            }
            else if (broj_optuznica) {
                    $(makediv).find(".textvalue").text(drzavnici_main_middle[0].broj_optuznica);
            }
            else {
        ...
        if (num_sort) {
            $('.isotope').prepend(makediv).isotope('reloadItems').isotope({
                sortBy: 'number'
            });
        }
        ...
        if (num_sort) {
            ...
            $(".isotope > .ikonice").each(function (i, obj) { 
                if (parseInt($(obj).find(".textvalue").text().trim()) < min) {
                    secondmin = min;
                    min = parseInt($(obj).find(".textvalue").text().trim());
                }
               else if (parseInt($(obj).find(".textvalue").text().trim()) < secondmin) {
                    secondmin = parseInt($(obj).find(".textvalue").text().trim());}
            });

            $(".isotope > .ikonice").each(function (i, obj) {
                if (secondmin == parseInt($(obj).find(".textvalue").text().trim())) {
                    indextofind = i;}
            });
            ...
            if (parseInt($(obj).find(".textvalue").text().trim()) == min && $(obj).find("img").attr("alt") != alt_atb) {   // and ako nije upravo dodan (bas taj koji se dodaje) uglavnom nemoze nikako eleminatri novog to je to nakon and-a
             
                if (filter_all) {
                    drzavnici_main_middle.splice(i, 1);
                    drzavnici_copy_copy.push($(this).find("img").attr("alt"));  //ulazak typehead objekta unutra.
                    ...
                    drzavnici_main_middle.splice(i, 1);
                    for (var i = 0; i < drzavnici.length; i++) {
                        if (drzavnici[i].imprezim.trim() == $(this).find("img").attr("alt").trim()) {
                            inner_drzavnici = {}
            ...
            if (filter_cro) {
                for (var i = 0; i < drzavnici.length; i++) {
                    if (drzavnici[i].imprezim.trim() == $(".isotope > .ikonice").eq(12).find("img").attr("alt").trim()) {
                        inner_drzavnici = {}
                        inner_drzavnici["newnames"] = drzavnici[i].imprezim;
                        drzavnici_cro.push(inner_drzavnici);
                    }
...
function setObjectAttributes(objectvalue) {
    ...
    first_name = objectvalue.trim().substring(0, objectvalue.indexOf(" ")).trim(); // Ivan
    last_name = objectvalue.trim().substring(objectvalue.indexOf(" "), objectvalue.length).trim(); //Šuker
    selected_item = (objectvalue.trim().toLowerCase()).replace(' ', '_');  //ivan_suker
...
function adjustMidlleIconsForTypeaheadCroPoliticians(array_one, array_two) {
    $(".isotope .ikonice").each(function (i, val) { array_one.push($(this).find("img").attr("alt")); });
    for (var i = array_one.length - 1; i < array_two.length; i++) { array_one.push("nan"); }
    for (var i = 0; i < 12; i++) {
